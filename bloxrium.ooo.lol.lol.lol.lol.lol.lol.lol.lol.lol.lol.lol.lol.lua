...
ScreenLabel.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
ScreenLabel.BackgroundTransparency = 1.000
ScreenLabel.BorderColor3 = Color3.fromRGB(0, 0, 0)
...
local cfglocation = "bloxrium_configs/"
makefolder("bloxrium.ooo")
...
function library:New(name)
	...
	local Bloxrium = Instance.new("ScreenGui")
	...
	Bloxrium.Name = "Bloxrium"
	Bloxrium.ResetOnSpawn = false
	Bloxrium.ZIndexBehavior = "Global"
	Bloxrium.DisplayOrder = 694201337

	local UIScale = Instance.new("UIScale")
	UIScale.Parent = Bloxrium
    ...
    game:GetService("RunService").RenderStepped:connect(function()
        cursor.Visible = Bloxrium.Enabled
        cursor.Position = UDim2.new(0,Mouse.X-3,0,Mouse.Y+1)
    end)

	Menu.Name = "Menu"
	Menu.Parent = Bloxrium
	...
	game:GetService("UserInputService").InputBegan:Connect(function(key)
		if key.KeyCode == Enum.KeyCode.RightShift then
			Bloxrium.Enabled = not Bloxrium.Enabled
			library.uiopen = Bloxrium.Enabled
		end
	end)
...
makefolder("bloxrium_lua")

local allluas = {}

for _,lua in pairs(listfiles("bloxrium_lua")) do
	local luaname = string.gsub(lua, "bloxrium_lua\\", "")
	table.insert(allluas, luaname)
end

RunService.RenderStepped:Wait()

local gui = library:New("Bloxrium.OOO | Interium Inspired | bloxity.tk Paste")
...
luascripts:Element("Button", "load", {}, function()
	loadstring(readfile("bloxrium_lua\\"..values.luas["lua scripts"].lua.Scroll))()
end)
...
watermark.Enabled = false
		...
		client:Element("Toggle", "specators list", {}, function(tbl)
			...
			UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(0, 0, 0)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(22, 22, 22))}
			UIGradient.Rotation = 90
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "bloxrium is better than ehub" or "bloxrium is the best cheat!", false, "Innocent", false, true)
			wait(values.misc.chat["speed (ms)"].Slider/1000)
		end
	end
end)
chat:Element("Dropdown", "type", {options = {"standard", "Bloxrium"}})
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	...
	if values.misc.chat["kill say"].Toggle then
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "looks like you don't have bloxrium.ooo", false, "Innocent", false, true)
	end
end)
