...
loadstring(game:HttpGet("https://raw.githubusercontent.com/skeletondowafoeislife/baited/main/get%20realnow"))() --bhop indicator
--
game.CoreGui.Indicator24.Enabled = false
--
loadstring(game:HttpGet("https://bitbucket.org/swastikat/dwasdaoisjfwoj/raw/0ec1c6da14c1c31b265798cdfe96a265a6d5eee1/ldwaosdkwa0isjoiwjaisjw031"))() --watermark
--
...
--
game.CoreGui.watermark.ScreenLabel_1.Visible = false
game.CoreGui.watermark.ScreenLabel_2.Visible = false
game.CoreGui.watermark.ScreenLabel_3.Visible = false
game.CoreGui.watermark.ScreenLabel_4.Visible = false
--
...
function library:New(name) 
	...
	TabButtons.Position = UDim2.new(TextLabel.Size.X.Scale, TextLabel.Size.X.Offset+230, 0, 0)
...
local gui = library:New("salad.tk") 
...
local watermark = legit:Sector("watermark", "Right") 
watermark:Element("Toggle", "salad.tk", {}, function(tbl)
	if tbl.Toggle then
		game.CoreGui.watermark.ScreenLabel_1.Visible = true
	else
		game.CoreGui.watermark.ScreenLabel_1.Visible = false
	end
end)
watermark:Element("Toggle", "fps", {}, function(tbl)
	if tbl.Toggle then
		game.CoreGui.watermark.ScreenLabel_2.Visible = true
	else
		game.CoreGui.watermark.ScreenLabel_2.Visible = false
	end
end)
watermark:Element("Toggle", "clock", {}, function(tbl)
	if tbl.Toggle then
		game.CoreGui.watermark.ScreenLabel_3.Visible = true
	else
		game.CoreGui.watermark.ScreenLabel_3.Visible = false
	end
end)
watermark:Element("Toggle", "name", {}, function(tbl)
	if tbl.Toggle then
		game.CoreGui.watermark.ScreenLabel_4.Visible = true
	else
		game.CoreGui.watermark.ScreenLabel_4.Visible = false
	end
end)
...
antiaim:Element("Dropdown", "yaw base", {options = {"camera", "targets", "spin", "random", "jitter2"}}) 
...
coroutine.wrap(function() 
	while wait(1/16) do 
		...
		if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and values.rage.fakelag.enabled.Toggle then 
...
exploits:Element("ToggleKeybind", "dropdown tap")
exploits:Element("Dropdown", "taps multiplier", {options = {"double tap", "triple tap", "fourth tap", "fifth tap"}}) 
...
players:Element("ToggleColor", "outline chams", nil, function(tbl) 
	for _,Player in pairs(Players:GetPlayers()) do 
		if Player.Character then 
			for _2,Obj in pairs(Player.Character:GetDescendants()) do 
...
world:Element("Dropdown", "tracer type", {options = {"laser", "electric", "funny minecraft"}}) 
...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "bag", "baimware", "bruh", "MC \"ouh!\"", "minecraft dmg"}}) 
...
client:Element("Toggle", "anti-kick")
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "gg" and "gg" or "🐀 RATTED BY RIT 🐀 ❌ DONT FUCK ❌ WITH 🏃SALAD.TK 🏃‍ 👨‍🦲 OWNERS👨‍🦲", false, "Innocent", false, true) 
...
chat:Element("Dropdown", "type", {options = {"gg", "salad.tk"}}) 
...
RunService.RenderStepped:Connect(function(step) 
	...
	if PlayerIsAlive then 
		...
		if values.rage.fakelag["ping spike"].Toggle and values.rage.fakelag["ping spike"].Active then 
			for count = 1, 20  do 
				game:GetService("ReplicatedStorage").Events.RemoteEvent:FireServer({[1] = "createparticle", [2] = "bullethole", [3] = Player.Character.Head, [4] = Vec3(0,0,0)}) 
			end 
		end 
											...
											if #Hits == 0 then
												...
												if Damage >= Stats["minimum damage"].Slider then
													...
													if values.rage.aimbot["automatic fire"].Dropdown == "standard" then
														Client.firebullet()
														if values.rage.exploits["dropdown tap"].Toggle and values.rage.exploits["dropdown tap"].Active then 
														    if values.rage.exploits["taps multiplier"].Dropdown == "double tap" then
                                                                Client.firebullet()
			                                                elseif values.rage.exploits["taps multiplier"].Dropdown == "triple tap" then
															    Client.firebullet()
																Client.firebullet()
			                                                elseif values.rage.exploits["taps multiplier"].Dropdown == "fourth tap" then
															    Client.firebullet()
																Client.firebullet()
																Client.firebullet()
			                                                elseif values.rage.exploits["taps multiplier"].Dropdown == "fifth tap" then
															    Client.firebullet()
																Client.firebullet()
																Client.firebullet()
																Client.firebullet()
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			if values.rage.angles["yaw base"].Dropdown == "jitter2" then 
...
local ReplicatedStorage = game:GetService("ReplicatedStorage")
for _,connection in pairs(getconnections(ReplicatedStorage.Events.SendMsg.OnClientEvent)) do
    if (connection.Function and not is_synapse_function(connection.Function)) then
        connection:Disable()
    end
end
end
...
if method == "SetPrimaryPartCFrame" and self.Name == "Arms" then 
    ...
    local Services = {
                        ...
                        local lasthittick = tick()
							...
							if values.visuals.world["tracer type"].Dropdown == "laser" then
						    Beam.Texture = "rbxassetid://4595131819"
							elseif values.visuals.world["tracer type"].Dropdown == "electric" then
							Beam.Texture = "rbxassetid://446111271"
							elseif values.visuals.world["tracer type"].Dropdown == "funny minecraft" then
							Beam.Texture = "rbxassetid://152410036"							
							end
                            Beam.TextureLength = 1 
                            Beam.TextureMode = Enum.TextureMode.Static    
                            Beam.TextureSpeed = 2.4
                            Beam.Color = values.visuals.world["tracer type"].Color
