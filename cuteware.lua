...
local cfglocation = "cw_config/"
makefolder("cw_config")
...
for _,lua in pairs(listfiles("cw_lua")) do
	local luaname = string.gsub(lua, "cw_lua\\", "")
	table.insert(allluas, luaname)
end

RunService.RenderStepped:Wait()

local gui = library:New("cuteware")
...
luascripts:Element("Button", "load", {}, function()
	loadstring(readfile("cw_lua\\"..values.luas["lua scripts"].lua.Scroll))()
end)
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "buy cuteware.xyz", false, "Innocent", false, true)
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	...
	if values.misc.chat["kill say"].Toggle then
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "looks like you don't have cuteware.xyz", false, "Innocent", false, true)
	end
end)
