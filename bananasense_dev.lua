...
library:CreateWatermark("bananasense | dev  | E")
...
function library:New(name) 
	...
	do 
		...
		TextLabel.BorderColor3 = COL3RGB(255, 249, 31) 
	...
	function menu:Tab(text) 
		...
		TextButton.MouseButton1Down:Connect(function() 
			if currenttab ~= text then 
				...
				library:Tween(TextButton, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 0)}) 
				currenttab = text 
				TabGui.Visible = true 
			end 
		end) 
		...
		function Tab:MSector(text, side) 
			...
			function MSector:Tab(text) 
				...
				TextButton.MouseButton1Down:Connect(function() 
					...
					tab1.Visible = true 
					library:Tween(TextButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 0)}) 
				end) 
				...
				function tab:Element(type, text, data, callback) 
					...
					if type == "Jumbobox" then 
						...
						Drop.ZIndex = 5 
						Drop.ScrollBarImageColor3 = COL3RGB(255, 255, 0) 
							...
							do 
								...
								Button.MouseButton1Down:Connect(function() 
									if TBLFIND(Element.value.Jumbobox, v) then 
										...
										library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 0)}) 
									end 
									updatetext() 
							...
							for i,v in pairs(Drop:GetChildren()) do 
								if v.Name ~= "UIListLayout" then 
									if TBLFIND(val.Jumbobox, v.Name) then 
										v.TextLabel.TextColor3 = COL3RGB(255, 255, 0)
									else 
					...
					elseif type == "ToggleKeybind" then 
						...
						Color.Name = "Color" 
						Color.Parent = Button 
						Color.BackgroundColor3 = COL3RGB(255, 255, 0)
						...
						local function update() 
							if Element.value.Toggle then 
								tween = library:Tween(Color, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = COL3RGB(255, 255, 0)}) 
								library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 255)}) 
							else 
								keybindremove(text) 
						...
						do 
							...
							Always.Text = "Always" 
							Always.TextColor3 = COL3RGB(255, 255, 0)
							...
							for _,button in pairs(Frame:GetChildren()) do 
								if button:IsA("TextButton") then 
									button.MouseButton1Down:Connect(function() 
										...
										button.TextColor3 = COL3RGB(255, 255, 0) 
										values[tabname][sectorname][tabtext][text] = Element.value 
										callback(Element.value) 
									end) 
									button.MouseEnter:Connect(function() 
										if Element.value.Type ~= button.Text then 
											library:Tween(button, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 0)}) 
										end 
									end) 
									button.MouseLeave:Connect(function() 
										if Element.value.Type ~= button.Text then 
					...
					elseif type == "ToggleTrans" then 
						...
						TextLabel.Parent = Button 
						TextLabel.BackgroundColor3 = COL3RGB(255, 255, 0) 
						TextLabel.BackgroundTransparency = 1.000 
						TextLabel.Position = UDIM2(0, 32, 0, -1) 
						TextLabel.Size = UDIM2(0.111913361, 208, 1, 0) 
						TextLabel.Font = Enum.Font.SourceSansSemibold 
					...
					elseif type == "Slider" then 
						...
						UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, COL3RGB(255, 255, 0)), ColorSequenceKeypoint.new(1, COL3RGB(255, 255, 0))} 
					...
					elseif type == "Button" then 
						...
						Button_2.MouseButton1Down:Connect(function() 
							TextLabel.TextColor3 = COL3RGB(255, 255, 0)
						...
						Button_2.MouseEnter:Connect(function() 
							library:Tween(TextLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 0)}) 
						end) 
			...
			function Sector:Element(type, text, data, callback) 
				...
				if type == "ScrollDrop" then 
					...
					for i,v in ipairs(joe) do 
						do 
							...
							Frame.ScrollBarImageColor3 = COL3RGB(255, 255, 0)      
								...
								for _,bad in pairs(v.Frame:GetChildren()) do      
									if bad:IsA("TextButton") then      
										...
										if bad.Name == Element.value.Scroll[v.Name] then      
											bad.TextLabel.TextColor3 = COL3RGB(255, 255, 0)     
										end      
									end      
								end      
							end      
				...
				elseif type == "Scroll" then      
					...
					function Element:SetValue(val)      
						...
						library:Tween(Frame[Element.value.Scroll].TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 0)})      
						values[tabname][sectorname][text] = Element.value      
						callback(Element.value)      
					end      
					values[tabname][sectorname][text] = Element.value      
...
makefolder("Bananalua") 

local allluas = {} 

for _,lua in pairs(listfiles("Bananalua")) do 
	local luaname = GSUB(lua, "Bananalua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("BananaSense") 
...
local credits = gui:Tab("Made By rage#8164")
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("Bananalua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
...
aimbot:Element("Jumbobox", "resolver", {options = {"banana"}}) 
...
antiaim:Element("Dropdown", "pitch", {options = {"up", "down", "imposter", "random", "banana",}}) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Winning" and "Get tapped or get BananaSense$$$" or "Join BananaSense", false, "Innocent", false, true) 
...
RunService.RenderStepped:Connect(function(step) 
	...
	if PlayerIsAlive then 
			...
			if values.rage.aimbot.enabled.Toggle then 
				...
				for _,Player in pairs(Players:GetPlayers()) do 
					...
					if Player.Character and Player.Character:FindFirstChild("Humanoid") and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and Player ~= LocalPlayer then 
						if TBLFIND(values.rage.aimbot.resolver.Jumbobox, "banana") then 
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "banana" and -6 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-10, 10)/10 or 2.5
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	...
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "Eating Banana" and values.misc.chat["message"].Text or "Banana is the god", false, "Innocent", false, true) 
	end 
end) 
