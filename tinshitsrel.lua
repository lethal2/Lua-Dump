...
function library:New(name)  
	...
	function menu:Tab(text)  
		...
		function Tab:MSector(text, side) 
			...
			Section.Size = UDIM2(0.95, 0, 0, 22)

			SectionText.Name = "SectionText" 
			SectionText.Parent = Section 
			SectionText.BackgroundColor3 = Color3.fromRGB(255, 255, 255) 
			SectionText.BackgroundTransparency = 1.000 
			SectionText.Position = UDIM2(0, 0, 0, -4) 
			SectionText.Size = UDIM2(1, 0, 0, 19) 
			...
			function MSector:Tab(text)  
				...
				local tabsize = UDIM2(0.95, 0, 0, 44)  
				...
				TextButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)  
				TextButton.BackgroundTransparency = 1.000  
				TextButton.Size = UDIM2(0, txt:GetTextSize(text, 14, Enum.Font.Ubuntu, Vector2.new(700,700)).X + 2, 1, 0)  
				TextButton.Font = Enum.Font.Ubuntu
				TextButton.Text = text  
				...
				function tab:Element(type, text, data, callback)  
					...
					elseif type == "ToggleKeybind" then  
						...
						do  
							...
							Keybind.Position = UDIM2(1, 0, 0.5, 0)  
							Keybind.Text = "[...]"  
							Keybind.Size = UDIM2(0, 43, 0, 12)  
							Keybind.Size = UDIM2(0,txt:GetTextSize("[...]", 14, Enum.Font.Ubuntu, Vector2.new(700, 12)).X + 5,0, 12)  
							...
							Keybind.MouseButton1Down:Connect(function()  
								if not binding then  
									...
									binding = true  
									Keybind.Text = "..."  
									Keybind.Size = UDIM2(0,txt:GetTextSize("...", 14, Enum.Font.Ubuntu, Vector2.new(700, 12)).X + 4,0, 12)  
								end  
							end)  
							...
							game:GetService("UserInputService").InputBegan:Connect(function(input)  
								...
								if binding then  
									...
									Keybind.Size = UDIM2(0,txt:GetTextSize(Keybind.Text, 14, Enum.Font.Ubuntu, Vector2.new(700, 12)).X + 5,0, 12)  
									...
									if input.KeyCode.Name == "Backspace" then  
										Keybind.Text = "NONE"  
										Keybind.Size = UDIM2(0,txt:GetTextSize(Keybind.Text, 14, Enum.Font.Ubuntu, Vector2.new(700, 12)).X + 4,0, 12)  
					...
					elseif type == "Slider" then  
						...
						local val  
						Button.MouseButton1Down:Connect(function()  
							Frame.Size = UDIM2(0, math.clamp(mouse.X - Frame.AbsolutePosition.X, 0, 175), 0, 10)  
...
for _,lua in pairs(listfiles("tinlua")) do 
...
luascripts:Element("lmao", "Player", {options = loopkillplr, Amount = 20})
luascripts:Element("Toggle", "Loop kill for hexagon retards", nil, function(tbl)
if tbl.Toggle then
...
local step2
	...
	--values.luas.luascripts.Player.Dropdown
				if Players[values.luas["lua scripts"].Player.Dropdown].Character and Players[values.luas["lua scripts"].Player.Dropdown].Team ~= LocalPlayer.Team and Players[values.luas["lua scripts"].Player.Dropdown].Character:FindFirstChild("UpperTorso") then
                game:GetService("ReplicatedStorage").Events.HitPart:FireServer(
                    Players[values.luas["lua scripts"].Player.Dropdown].Character.Head, -- 1
                    predict(Players[values.luas["lua scripts"].Player.Dropdown].Character.Head, game.Stats.PerformanceStats.Ping:GetValue()), --2
                    "Banana", --3
                    100, -- Range --4
...
luascripts:Element("Toggle", "Loop kill that uses stormy's kill all", nil, function(tbl)
if tbl.Toggle then
_G.Disable1 = false
local step1
	...
	--values.luas.luascripts.Player.Dropdown
				if Players[values.luas["lua scripts"].Player.Dropdown].Character and Players[values.luas["lua scripts"].Player.Dropdown].Team ~= LocalPlayer.Team and Players[values.luas["lua scripts"].Player.Dropdown].Character:FindFirstChild("UpperTorso") then
					local oh1 = Players[values.luas["lua scripts"].Player.Dropdown].Character.Head
					local oh2 = Players[values.luas["lua scripts"].Player.Dropdown].Character.Head.CFrame.p
	...
	if PlayerIsAlive then 
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			if values.rage.angles["body roll"].Dropdown == "180" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "180" and RAD(values.rage.angles["body roll offset"].Slider) or 0, 1, 0)  
