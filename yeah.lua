...
library:CreateWatermark("RICKHACK | V1")
...
function library:Tween(...) TweenService:Create(...):Play() end
         local cfglocation = "Rickcfg/"
         makefolder("Rickcfg")

         -- idc if u paste this just credit lucious#9999
         ...
         local MainUIColor = COL3RGB(237, 41, 57)

         --Credits to Rainy.solutions for the Watermark, MexicanHook for Show Team Damage GUI, making the Custom Pitch, Making the resolver, And prediction better, LunarSense for Most AA's, Bloxity.tk for Spectators List and Keybinds.
         ...
         function library:New(name)
                  ...
                  Menu.Image = "http://www.roblox.com/asset/?id=156646987"
                  ...
                  do
                           ...
                           TextLabel.BorderColor3 = COL3RGB(237, 41, 57)
...
repeat RunService.RenderStepped:Wait() until game:IsLoaded()
         ...
         repeat wait() until Models ~= nil
                  ...
                  repeat wait() until ChrModels ~= nil
                           ...
                           local gui = library:New("RICKHACK")
                           local legit = gui:Tab("legit")
                           local rage = gui:Tab("rage")
                           local visuals = gui:Tab("visuals")
                           local visuals2 = gui:Tab("players")
                           local misc = gui:Tab("misc")
                           local hack = gui:Tab("server-side")
...
local function AddLegit(Tab)
         ...
         Tab:Element("Slider", "field of view", {min = 30, max = 180, default = 120})
         Tab:Element("Slider", "smoothing", {min = 1, max = 50, default = 1})
         Tab:Element("Toggle", "silent aim")
         Tab:Element("Slider", "hitchance", {min = 1, max = 100, default = 50})
         ...
         Tab:Element("Slider", "minimum dmg", {min = 0, max = 180, default = 15})
end
...
aimbot:Element("Dropdown", "prediction type", {options = {"off", "Rickhack", "bloxsense.gay", "ova.rocks", "nixus"}})
...
aimbot:Element("Dropdown", "autoshoot type", {options = {"Rick" ,"standard", "off"}})
aimbot:Element("Dropdown", "autowall type", {options = {"Rick", "standard", "off"}})
...
antiaim:Element("Dropdown", "pitch", {options = {"None", "Up", "Down","Huge", "Custom", "Zero", "180", "Random AA", "Rick", "God", "Sus_down", "wtf", "glitch", "sussy", "fucker"}})
...
antiaim:Element("Slider", "spin speed", {min = 1, max = 359, default = 4})
...
Peek = false
local exploits = hack:Sector("exploits", "Left")
exploits:Element("ToggleKeybind", "triple tap")
exploits:Element("Dropdown", "triple tap type", {options = {"Rapid", "Standard", "Instant"}})
...
if tbl.Toggle and tbl.Active and LocalPlayer.Character and Peek == false then
         if values.hack.exploits["loop peek"].Toggle == true and values.hack.exploits["quick peek"].Active == true then
                  while values.hack.exploits["loop peek"].Toggle == true do
                           Peek = true
                           LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,values.hack.exploits["quick peek vertical pos"].Slider,0)
                           wait(0.2)
                           Peek = false
                           LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame - Vector3.new(0,values.hack.exploits["quick peek vertical pos"].Slider,0)
                           wait(0.2)
...
while values.hack.exploits["debris clear"].Toggle == true do
         wait(1)
...
spawn(function()
while values.hack.exploits["fakeframe"].Toggle == true and values.hack.exploits["fakeframe"].Active == true do
         wait()
         game:GetService("NetworkClient"):SetOutgoingKBPSLimit(1)
end
game:GetService("NetworkClient"):SetOutgoingKBPSLimit(9e9)
...
if values.hack.exploits["auto peek"].Toggle and values.hack.exploits["auto peek"].Active and LocalPlayer.Character then
...
local players = visuals2:Sector("players", "Left")
...
if Player.Character then
         for _2,Obj in pairs(Player.Character:GetDescendants()) do
                  if Obj.Name == "VisibleCham" or Obj.Name == "WallCham" then
                           Obj.AlwaysOnTop = values.visuals2.players["invisible chams"].Toggle
                           if tbl.Toggle then
...
effects:Element("Toggle", "better shadows", nil, function(val) sethiddenproperty(Lighting, "Technology", val.Toggle and "ShadowMap" or "Legacy") end)

local self = visuals2:Sector("self", "Right")
self:Element("ToggleKeybind", "third person", {}, function(tbl)
if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
if tbl.Toggle then
         if tbl.Active then
                  LocalPlayer.CameraMaxZoomDistance = values.visuals2.self.distance.Slider
                  LocalPlayer.CameraMinZoomDistance = values.visuals2.self.distance.Slider
                  LocalPlayer.CameraMaxZoomDistance = values.visuals2.self.distance.Slider
                  LocalPlayer.CameraMinZoomDistance = values.visuals2.self.distance.Slider
         else
...
if values.visuals2.self["on scope"].Toggle or not LocalPlayer.Character:FindFirstChild("AIMING") then
...
ViewmodelOffset = CF(values.visuals2.self["viewmodel x"].Slider/7, values.visuals2.self["viewmodel y"].Slider/7, values.visuals2.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals2.self.roll.Slider/50)
end)
...
for _,obj in pairs(SelfObj) do
         if obj.Parent ~= nil then
                  obj.Material = values.visuals2.self["self chams material"].Dropdown
                  obj.Color = values.visuals2.self["self chams"].Color
                  obj.Transparency = values.visuals2.self["self chams transparency"].Slider/10
         end
end
else
for _,obj in pairs(SelfObj) do
...
for _,part in pairs(LocalPlayer.Character:GetDescendants()) do
         if part:IsA("Part") or part:IsA("MeshPart") then
                  if part.Transparency ~= 1 then
                           part.Transparency = LocalPlayer.Character:FindFirstChild("AIMING") and values.visuals2.self["scope blend"].Slider/100 or 0
                  end
         end
         if part:IsA("Accessory") then
                  part.Handle.Transparency = LocalPlayer.Character:FindFirstChild("AIMING") and values.visuals2.self["scope blend"].Slider/100 or 0
         end
end
end
end)()
...
local configs = settings:Sector("configs", "Right")
configs:Element("TextBox", "config", {placeholder = "config namer yes"})
configs:Element("Button", "save", {}, function() if values.settings.configs.config.Text ~= "" then library:SaveConfig(values.settings.configs.config.Text) end end)
configs:Element("Button", "load", {}, function() if values.settings.configs.config.Text ~= "" then ConfigLoad:Fire(values.settings.configs.config.Text) end end)
...
local chat = settings:Sector("chat", "Left")
chat:Element("Toggle", "chat spam", nil, function(tbl)
if tbl.Toggle then
while values.settings.chat["chat spam"].Toggle do
game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.settings.chat.type.Dropdown == "Winning" and "RICKHACK WINNING $$$$$" or values.settings.chat.type.Dropdown == "RICK" and "RICK RICK RICK RICK RICK RICK RICK" or values.settings.chat.type.Dropdown == "MOM" and "I FUCKED UR MOM NIGGA I FUCKED UR MOM NIGGA I FUCKED UR MOM NIGGA" or values.settings.chat.type.Dropdown == "SEMI" and "🐀 RATTED BY SEMI  🐀 ❌ DONT FUCK ❌ WITH 👾BLOXSENSE 👾👨‍🦲USERS👨‍🦲", false, "Innocent", false, true)
wait(values.settingd.chat["speed (ms)"].Slider/1000)

end
end
end)
chat:Element("Dropdown", "type", {options = {"Winning", "RICK", "MOM", "SEMI"}})
...
local ui = settings:Sector("GUI", "Left")
ui:Element("Toggle", "scaling")
ui:Element("Slider", "amount", {min = 3, max = 13, default = 10})
...
local griefsector = settings:Sector("Server Side","Left")
griefsector:Element("Button","1HP",{},function()
...
CreateHitElement("RICKHACK",Color3.new(0,1,0))
ChatScript.moveOldMessages()
ChatScript.createNewMessage("RICK ASTLEY","RICKHACK HAS LOADED",MainUIColor,Color3.new(1,1,1),0.01,nil)
ChatScript.moveOldMessages()
ChatScript.createNewMessage("RICK ASTLEY","THIS CHEAT DOESNT DUMP (REAL)",MainUIColor,Color3.new(1,1,1),0.01,nil)
ChatScript.moveOldMessages()
ChatScript.createNewMessage("lucious#9999","i totally didnt make this",MainUIColor,Color3.new(1,1,1),0.01,nil)
ChatScript.moveOldMessages()
ChatScript.createNewMessage("lucious#9999","rickhack is one of the best cheats known to man.",MainUIColor,Color3.new(1,1,1),0.01,nil)
Print("enjoy RICKHACK pasted by lucious#9999")