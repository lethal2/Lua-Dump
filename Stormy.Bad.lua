...
local cfglocation = "Stormy.Badcfg/"    
makefolder("Stormy.Badcfg")    
...
function library:New(name)    
	...
	Menu.Image = "http://www.roblox.com/asset/?id=7761801902"    
...
makefolder("Stormy.Badlua")    
  
local allluas = {}    
  
for _,lua in pairs(listfiles("Stormy.Badlua")) do    
	local luaname = GSUB(lua, "Stormy.Badlua\\", "")    
	INSERT(allluas, luaname)    
end    
  
RunService.RenderStepped:Wait()    
  
local gui = library:New("Stormy.Bad")    
...
luascripts:Element("Button", "load", {}, function()    
	loadstring(readfile("Stormy.Badlua\\"..values.luas["lua scripts"].lua.Scroll))()    
end)    
...
local function AddRage(Tab)    
	...
	Tab:Element("Slider", "max fov", {min = 1, max = 360, default = 180})    
end    
...
antiaim:Element("Slider", "spin speed", {min = 1, max = 165, default = 4})   
...
exploits:Element("ToggleKeybind", "3 Tap")   
...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "bag", "baimware", "bubble with bell", "cod", "osu", "Tf2", "Tf2 pan", "M55solix", "Slap", "1", "Minecraft", "jojo", "vibe", "supersmash", "epic", "retro", "quek", "killing spree", "UwU", "Hit that clarence", "HAHA" }})    
world:Element("Slider", "sound volume", {min = 1, max = 7, default = 3})    
...
chat:Element("Toggle", "chat spam", nil, function(tbl)    
	if tbl.Toggle then    
		while values.misc.chat["chat spam"].Toggle do    
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Get Tapped By Stormy.Bad" and "Get Tapped By Stormy.Bad" or "локипосуда сверху", false, "Innocent", false, true)    
											...
											if #Hits == 0 then    
												...
												if Damage >= Stats["minimum damage"].Slider then    
													...
													if values.rage.aimbot["automatic fire"].Dropdown == "standard" then    
														Client.firebullet()    
														if values.rage.exploits["3 Tap"].Toggle and values.rage.exploits["3 Tap"].Active then    
															Client.firebullet() Client.firebullet()    
														end    
...
mt.__namecall = function(self, ...)    
	...
	if method == "FireServer" and self.Name == "HitPart" then    
		...
		if values.rage.aimbot["automatic penetration"].Toggle and values.rage.aimbot["autowall type"].Dropdown == "buggy" then end
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)    
	...
	if values.misc.chat["kill say"].Toggle then    
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "🤡 Get Tapped By Stormy.Bad 🤡", false, "Innocent", false, true)    
	end    
end)    
