...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "sit down", "semi", "baimware"}})     
...
movement:Element("Slider", "speed", {min = 5, max = 100, default = 40})      
...
grenades:Element("Toggle", "anti-lag", {}, function(tbl)      
			spawn(function()
				while values.misc.grenades["anti-lag"].Toggle do
					pcall(function()
						game:GetService("RunService").RenderStepped:Wait()
						for i,v in pairs(workspace["Ray_Ignore"]:GetChildren()) do
							if v.Name == "MagDrop" then
...
RunService.RenderStepped:Connect(function(step)      
	...
	if PlayerIsAlive then      
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then      
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and CamLook.Y or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "zero" and math.huge or 10.10 and values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-10, 10)/10 or 2.5      
...
LocalPlayer.Cash:GetPropertyChangedSignal("Value"):Connect(function(cash)      
	if values.misc.client["infinite cash"].Toggle and cash ~= 90000000000 then      
		LocalPlayer.Cash.Value = 90000000000
	end      
end)      
